  ...
  $stateProvider
    .state(
      'sports', {
        ...
        controller: function(sportsService, $location) {
          ...
          this.isActive = (sportNameToCheck) => {
            const pathRegexp = /sports\/(\w+)/;
            const match = pathRegexp.exec($location.path());
            if(match === null || match.length === 0) return false;
            let selectedSportName = match[1];
            return sportNameToCheck === selectedSportName;
          };
        },
    ...
    .state('sports.new', {
      ...
      controller: function($stateParams,  $state, $http) {
        ...
        this.addNewMedal = (medal) => {
          console.log('medal: ', medal);
          $http({method: 'POST', url: `/sports/${this.sportName}/medals`, data: {medal}})
          .then(() => {
